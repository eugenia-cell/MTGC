...
# 加载交通数据
traffic_data_path = 'Data/train_dataset/traffic_matrix.npy'
traffic_data = np.load(traffic_data_path)

# 处理交通数据
traffic_day = np.sum(traffic_data, axis=1)
traffic_week = np.sum(traffic_data, axis=0)
...
# 初始化特征张量，并移动到GPU
features = torch.randn(180, 144).cuda()
...
for epoch in range(num_epochs):
    ...
    for time_step, graph in enumerate(week_graph):
        optimizerW.zero_grad()
        s_embeddings = Wmodel(graph.x.cuda(), graph.edge_index.cuda(), graph.edge_attr.cuda(), w_time_diff, week_segments[time_step % 7])
