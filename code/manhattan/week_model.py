...
class week_model(nn.Module):
    def __init__(self, in_channels, out_channels,num_time_features):
        ...
        self.num_time_features=num_time_features
        self.diffAttention = SelfAttention(144,144)
    ...
    def combine_week_features(self,week_series_embeddings):
        workday = sum(week_series_embeddings[9:16])/len(week_series_embeddings[9:16])
        weekend = sum(week_series_embeddings[19:24])/len(week_series_embeddings[19:24])
        difference = abs(workday,weekend) 
        week_combined = torch.stack([workday,weekend,difference])
        overall_integrated_representation = self.TemAttention(week_combined)
