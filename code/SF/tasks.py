...
def lu_classify(emb,display=False):
    lu_label_filename="Data/eval_dataset/SF_region_labels.json"
    cd=json.load(open(lu_label_filename))
    cd_labels=np.zeros((194))
    for i in range(194):
...
def do_tasks(embs,display=True):
    ...
    crime_count_label=np.load("Data/eval_dataset/crime_counts.npy")
    ...
    check_in_label=np.load("Data/eval_dataset/Region_Checkin_Counts.npy")
