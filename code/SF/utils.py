...
def day_mob_s_t(traffic_day):
    # 假设 traffic_matrix 是 (24, 194, 194) 形状的矩阵，表示流量数据
...
def pairwise_inner_product(mat_1, mat_2):
    n, m = mat_1.shape  # (194, 144)
    mat_expand = torch.unsqueeze(mat_2, 0)  # (1, 194, 144),
    mat_expand = mat_expand.expand(n, n, m)  # (194, 194, 144),
    mat_expand = mat_expand.permute(1, 0, 2)  # (194, 194, 144),
    inner_prod = torch.mul(mat_expand, mat_1)  # (194, 194, 144), 
    inner_prod = torch.sum(inner_prod, axis=-1)  # (194, 194),
    return inner_prod
