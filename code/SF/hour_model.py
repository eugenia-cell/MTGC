...
class hourAttention(nn.Module):
    def __init__(self, embed_dim):
        super(hourAttention, self).__init__()
...
class hour_model(nn.Module):
    def __init__(self, in_channels, out_channels,num_time_features):
        ...
        self.attention = hourAttention(out_channels)
    ...
    def combine_hour_features(self,hour_series_embeddings):
        ...
        #overall_integrated_representation = self.DIFF_fusion(hour_series_embeddings)
        #return overall_integrated_representation
        morningpeak_mean = sum(hour_series_embeddings[6:9])/len(hour_series_embeddings[6:9])
        eveningpeak_mean = sum(hour_series_embeddings[16:19])/len(hour_series_embeddings[16:19])
        nightime_mean = sum(hour_series_embeddings[19:24])/len(hour_series_embeddings[19:24]) + sum(hour_series_embeddings[0:6])/len(hour_series_embeddings[0:6])
        daytime_mean = sum(hour_series_embeddings[9:16])/len(hour_series_embeddings[9:16])
